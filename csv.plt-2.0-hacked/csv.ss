...
(define (csv-map proc reader-or-in)
  (let ((reader (%csv:reader-or-in-arg "csv-map" reader-or-in)))
    (let loop ([accum null])
      (let ([row (reader)])
        (if (null? row)
            (reverse accum)
            (loop (cons (proc row) accum)))))))
...
(define (csv->list reader-or-in)
  (csv-map (lambda (x) x) reader-or-in))
...
;;; 0.7.4.2 in @code{csv-internal:make-portreader/positional}.  Thanks to
